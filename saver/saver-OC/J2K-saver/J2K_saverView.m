...
- (void)initUI:(NSRect)rect {
    ...
    if (rect.size.width < 300 && rect.size.height < 200) {
        ...
        self.sdfText = [[NSTextField alloc] init];
        self.sdfText.frame = CGRectMake(widthPadding, lineHeight - 250, showWith, 200);
        self.sdfText.editable = NO;
        self.sdfText.bordered = NO;
        self.sdfText.drawsBackground = NO;
        self.sdfText.font = [NSFont systemFontOfSize:40];
        self.sdfText.textColor = jikeY;
        self.sdfText.alignment = NSTextAlignmentCenter;
        [self addSubview: self.sdfText];
        
    }
}

- (void)getDate {
    NSURL *url = [NSURL URLWithString:@"https://nondanee.gitlab.io/jike-daily-card-api/data.json"];
    ...
    NSURLSessionDataTask *dataTask = [session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        if (error == nil) {
            ...
            NSArray *cardData = dict[@"data"][@"cards"];
            NSDictionary *daily = cardData[0];
            NSString *fortune = daily[@"fortune"];
            NSDictionary *featuredContent = daily[@"featuredContent"];
            NSString *recommedDes = featuredContent[@"text"];
